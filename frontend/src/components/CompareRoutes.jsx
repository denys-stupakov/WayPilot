function CompareRoutes({
    ...
    return (
        <div
            ...
            <label className="block mb-1 text-sm text-gray-700">First Path</label>
            <select
                value={mode1}
                onChange={(e) => setMode1(e.target.value)}
                className="w-full mb-3 bg-white/40 rounded-lg p-2 focus:ring-2 focus:ring-blue-400"
            >
                <option value="normal">Shortest Path</option>
                <option value="trafficLights">Avoid Traffic Lights</option>
            ...
            <label className="block mb-1 text-sm text-gray-700">Second Path</label>
            <select
                value={mode2}
