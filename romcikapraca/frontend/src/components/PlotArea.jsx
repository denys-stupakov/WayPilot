...
function buildNewtonStep(step, iterIndex, totalSteps){
  ...
  const color = isFirst ? '#d40000' : stepColor(iterIndex, totalSteps, 1);
  ...
  if(dfx != null && isFinite(dfx) && Math.abs(dfx) > 1e-10){
    ...
    // Касательная: от (xn, fx) до (xnext, 0)
    traces.push({
      x: [xn, xnext], y: [fy, 0],
      mode: 'lines',
  ...
  useEffect(()=>{
    ...
    if(data.single_mode){
      traces.push({
        ...
        line: {color: '#2196f3', width: 3, shape: 'linear'},
      ...
      // use_derivative_colors: шаг 5 итераций использует те же цвета что и шаг 3
      const useDerivColors = data.derivatives_mode || data.use_derivative_colors;
      const color1 = useDerivColors ? '#00c875' : colorFunc1;
      const color2 = useDerivColors ? '#ffeb3b' : colorFunc2;
      ...
      if(isHZero && !data.derivatives_mode && data.m_value == null && !data.use_derivative_colors){
        traces.push({
          x: [xMin, xMax], y: [0, 0], mode: "lines",
    ...
    if(newton?.root !== undefined && !data.derivatives_mode){
      traces.push({
        ...
        name: "Nájdený koreň", showlegend: true,
        hovertemplate: `<b>Koreň α</b><br>x = %{x:.8f}<extra></extra>`,
      });
    }

    // Маркер m (только шаг 5 iterations)
    if(data.m_value != null && data.m_x != null && data.m_y != null){
      const mYabs = Math.abs(data.m_y);
      traces.push({
        x: [data.m_x], y: [mYabs],
        mode: "markers+text",
        ...
        hovertemplate: `<b>m = min|f′(x)| = ${mYabs.toFixed(6)}</b><extra></extra>`,
      });
    }

    // Начальная точка x₀ на оси x (iterations mode) — показывается сразу при загрузке шага 5
    if(data.x0_init != null && isFinite(data.x0_init)){
      traces.push({
        x: [data.x0_init], y: [0],
        mode: 'markers+text',
        marker: { color: '#d40000', size: 10, symbol: 'circle', line: {color: '#111', width: 1.5} },
        text: ['x<sub>0</sub>'],
        textposition: 'bottom center',
        textfont: { color: '#d40000', size: 11, family: 'Arial Black, Arial' },
        name: 'x₀', showlegend: false,
        legendgroup: 'x0_init',
        hovertemplate: `<b>x₀ = ${data.x0_init.toFixed(6)}</b><extra></extra>`,
      });
    }
