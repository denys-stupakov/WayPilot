...
export default function Step1Input({fieldRef, calculationMode, data, onSubmit, onInsertLatex, onInsertFrac}){

  return(
    <div>
      <div className="math-field-container">
        <math-field ref={fieldRef} placeholder="napr. x^2-4x-10=0" />
      </div>
      <div className="math-buttons-grid">
        ...
        <button onClick={() => onInsertLatex("^")} className="math-button">^</button>
      ...
      {data?.g_expr && data?.h_expr && (
        <div style={{
          ...
          <div style={{
            fontSize: '12px', color: 'var(--color-axis)',
            marginBottom: '10px', textAlign: 'center', fontWeight: 'bold'
          }}>
          ...
          <div style={{ marginBottom: '8px' }}>
            <div style={{
              padding: '8px 12px', background: 'rgba(33,150,243,0.12)',
              border: '2px solid var(--color-func1)', borderRadius: '6px',
              fontFamily: 'serif', fontSize: '16px', color: 'var(--color-func1)',
            }}>
              g(x)<LatexDisplay latex={data.g_latex || data.g_expr} style={{ color: 'var(--color-func1)', fontSize: '16px' }} />
            </div>
          </div>
          ...
          <div>
            <div style={{
              padding: '8px 12px', background: 'rgba(255,152,0,0.12)',
              border: '2px solid var(--color-func2)', borderRadius: '6px',
              fontFamily: 'serif', fontSize: '16px', color: 'var(--color-func2)',
            }}>
              h(x)<LatexDisplay latex={data.h_latex || data.h_expr} style={{ color: 'var(--color-func2)', fontSize: '16px' }} />
            </div>
          </div>

          <div style={{
            marginTop: '10px', padding: '8px', background: 'rgba(0,230,118,0.08)',
            borderRadius: '6px', fontSize: '12px', color: 'var(--color-axis)',
